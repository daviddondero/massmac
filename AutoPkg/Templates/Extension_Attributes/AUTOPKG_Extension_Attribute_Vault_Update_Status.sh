#!/bin/bash
##########################################################################
# Jamf Extension Attribute: AutoPkg Vault Update Status
#
# Purpose:
#   Checks whether Vault is installed and up-to-date.
#   Compares the installed version against AutoPkg latest version (%version%).
#
# Functionality:
#   1. Checks /usr/local/bin/vault or /opt/homebrew/bin/vault.
...
# ----------------------------------------------------------------------
# Locate installed Vault and read version
# ----------------------------------------------------------------------
for p in "${VAULT_PATHS[@]}"; do
    if [ -x "$p" ]; then
        APP_PATH="$p"
        INSTALLED_VERSION=$("$APP_PATH" --version 2>&1 | awk '{print $2}' | sed 's/^v//')
        log "Found $APP_NAME at $APP_PATH, version $INSTALLED_VERSION"
        break
    fi
done
