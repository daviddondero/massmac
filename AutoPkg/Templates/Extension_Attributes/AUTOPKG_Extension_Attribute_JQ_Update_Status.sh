#!/bin/bash
##########################################################################
# Jamf Extension Attribute: AutoPkg jq Update Status
#
# Purpose:
#   Checks whether jq is installed and up-to-date.
#   Compatible with AutoPkg by using %NAME% for the app name and %version%
#   for the latest available version.
#
# Functionality:
#   1. Searches common jq locations:
#        - /usr/local/bin/jq
#        - /opt/homebrew/bin/jq
...
#   5. Removes *all* existing JSON files for jq before writing the new one.
...
#
# Usage:
#   AutoPkg substitutes %NAME% and %version% automatically when the script
#   runs as part of a recipe.
##########################################################################
...
# ----------------------------------------------------------------------
# Locate installed jq binary and log path
# ----------------------------------------------------------------------
POSSIBLE_PATHS=(
    "/usr/local/bin/jq"
    "/opt/homebrew/bin/jq"
...
# ----------------------------------------------------------------------
if [ -n "$APP_PATH" ]; then
    INSTALLED_VERSION=$("$APP_PATH" --version 2>&1 | sed 's/^jq-//')
